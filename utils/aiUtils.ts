...
const fallbackResponse = (userMessage: string, companion: any) => {
  ...
  if (questions.some(question => lowerMessage.includes(question))) {
    if (lowerMessage.includes('how are you')) {
      return `I'm doing well, thank you for asking! I've been thinking about ${
        companion.personalityTraits[0]?.name === 'Creative' ? 'some new creative ideas' : 'interesting things'
  ...
  const hasPersonalityTrait = (traitName: string) => {
    return companion.personalityTraits.some((trait: any) => trait.name === traitName);
  };
...
export const generateAIResponse = async (
  ...
  if (openai) {
    try {
      ...
      console.warn('OpenAI request failed, falling back to local responses.', err);
    }
  }
