...
export const generateAIResponse = async (userMessage: string, companion: any) => {
  ...
  // In a real app, this would connect to an AI service like OpenAI GPT

  const greetings = ['hello', 'hi', 'hey', 'what\'s up', 'greetings'];
  const questions = ['how are you', 'what are you', 'who are you', 'what can you do'];
  ...
  if (questions.some(question => lowerMessage.includes(question))) {
    if (lowerMessage.includes('how are you')) {
      return `I'm doing well, thank you for asking! I've been thinking about ${companion.personalityTraits[0]?.name === 'Creative' ? 'some new creative ideas' : 'interesting things'} lately. How about you?`;
    }
  ...
  if (lowerMessage.includes('nsfw') || lowerMessage.includes('sex') || lowerMessage.includes('nude')) {
    if (companion.nsfwEnabled) {
  ...
  // Default responses based on personality traits
  const hasPersonalityTrait = (traitName: string) => {
    return companion.personalityTraits.some(trait => trait.name === traitName);
  };
